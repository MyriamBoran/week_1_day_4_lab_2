...
class TestFriends < MiniTest::Test
  ...
  def test_add_friend
    ...
    assert_equal(old_length + 1, @person4[:friends].length)
  end
  ...
  def test_remove_friend
    old_length = @person3[:friends].length
    result = remove_friend(@person3, "Velma")
    assert_equal(old_length - 1, @person3[:friends].length)
  end
  ...
  def test_loan_value
    old_money_a = @person5[:monies]
    old_money_b = @person4[:monies]
    loan_value(@person5, @person4, 30)
    assert_equal(true, @person5[:monies] < old_money_a)
    assert_equal(true, @person4[:monies] > old_money_b)
  end
